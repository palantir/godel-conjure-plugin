...
`
	...
	for _, outputName := range []string{"conjure-output", "conjure-output2"} {
		contentBytes, err := os.ReadFile(path.Join(projectDir, outputName, "conjure", "test", "api", "structs.conjure.go"))
		require.NoError(t, err)
...
`
	...
	for _, outputName := range []string{"conjure-output"} {
		contentBytes, err := os.ReadFile(path.Join(projectDir, outputName, "conjure", "test", "api", "unions.conjure.go"))
		require.NoError(t, err)
		wantContent := `// Code generated by conjure-go. DO NOT EDIT.
...
`
	...
	err = os.MkdirAll(path.Join(projectDir, "godel", "config"), 0755)
	require.NoError(t, err)
	err = os.WriteFile(path.Join(projectDir, "godel", "config", "conjure-plugin.yml"), []byte(`
projects:
  project-1:
    output-dir: conjure
    ir-locator: `+yamlDir+`
