...
type LoadedAssets struct {
	ConjureExtensionsProviders []conjureextensionsprovider.Generator
...
func LoadAssets(assets []string) (LoadedAssets, error) {
	...
	// create Conjure extensions generators from assetapi.ConjureExtensionsProvider assets
	var conjureExtensionsProviders []conjureextensionsprovider.Generator
	for _, assetPath := range assetTypeToAssetsMap[assetapi.ConjureExtensionsProvider] {
		conjureExtensionsProviders = append(conjureExtensionsProviders, conjureextensionsprovider.NewAssetProvider(assetPath))
	}

	return LoadedAssets{
		ConjureIRExtensionsProviders: assetTypeToAssetsMap[assetapi.ConjureIRExtensionsProvider],
		ConjureExtensionsProviders:   conjureExtensionsProviders,
	}, nil
}
...
func createAssetTypeToAssetsMap(assets []string) (map[assetapi.AssetType][]string, error) {
	validAssetTypes := assetapi.AllAssetsTypes()
...
func getAssetTypeForAsset(asset string) (assetapi.AssetType, error) {
	...
	assetType, assetTypeErr := cmdutils.GetCommandOutputAsJSON[string](exec.Command(asset, assetapi.AssetTypeCommand))
	if assetTypeErr == nil {
